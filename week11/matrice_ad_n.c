...
int preluareGraf(const char *nume, int *nrv, int *nrm, int g[][3])
{
    int er, i;
    FILE *f;
    f = fopen(nume, "r");
...
void conversieTableToMatrice(int nrv, int nrm, int tabel[][3], int mat[6][6], int *dim)
{
    int i, j;
    *dim = nrv;
...
int main()
{
    int nrv, nrm, mat[6][6], tabel[8][3], er, dim;
    // mat[][] matricea de adicaenta
    // tabel forma tabelara
    // dim matricei
    er = preluareGraf("g1p.txt", &nrv, &nrm, tabel);
    if (er == 1)
    {
